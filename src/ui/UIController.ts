...
export class UIController {
  ...
  private renderPalette(): void {
    ...
    palette.innerHTML = this.order.availableModuleIds.map((moduleId) => {
  ...
  private renderModuleDetail(instance: ModuleInstance | undefined): void {
    ...
    if (!instance) { panel.innerHTML = `<p class="empty-state">공장 속 장치를 선택하면 역할과 연결 포트를 설명해 드립니다.</p>`; return; }
    const definition = modulesById.get(instance.moduleId)!;
    panel.innerHTML = `<div class="detail-title"><span>${definition.iconKey}</span><div><h3>${definition.displayName}</h3><p>${definition.description}</p></div></div><dl><div><dt>입력</dt><dd>${definition.inputPorts.length ? definition.inputPorts.map((port) => `${port.label} · ${port.dataType}`).join("<br>") : "없음"}</dd></div><div><dt>출력</dt><dd>${definition.outputPorts.map((port) => `${port.label} · ${port.dataType}`).join("<br>")}</dd></div><div><dt>처리 시간</dt><dd>${definition.processingTime ? `${definition.processingTime}초` : "즉시"}</dd></div></dl>`;
  }
...
function shell(): string {
  return `<main class="app-shell"><section id="menu-screen" class="menu-screen"><div class="brand-chip">AI FACTORY</div><h1>AI Factory</h1><p class="hero-copy">주문서를 읽고 장치를 연결해<br>더 좋은 그림을 납품하는 공장 퍼즐</p><p id="menu-progress" class="menu-progress"></p><div class="menu-actions"><button id="start-game" class="primary">새 게임 시작</button><button id="continue-game" class="secondary">이어서 하기</button></div><p class="menu-note">생성형 AI 지식은 필요 없습니다. 결과를 보고 공장을 개선해 보세요.</p></section><section id="game-screen" class="game-screen is-hidden"><header class="topbar"><div><span class="brand-chip small">AI FACTORY</span><span id="order-step" class="order-step">1 / 6</span></div><div class="top-actions"><span id="credits" class="credits">0 C</span><span id="factory-status" class="factory-status">공장 대기 중</span><button id="undo-action" class="ghost">↶ 되돌리기</button><button id="reset-factory" class="ghost">초기화</button><button id="clear-save" class="ghost danger">저장 초기화</button><button id="back-to-menu" class="ghost">메뉴</button></div></header><div class="game-layout"><aside class="left-panel panel"><div class="panel-heading"><span>장치 보관함</span><small>클릭해 공장에 배치</small></div><div id="module-palette" class="module-palette"></div></aside><section class="factory-column"><div class="order-card"><div><span class="eyebrow">현재 의뢰</span><h2 id="order-title"></h2><p id="order-request"></p></div><button id="produce" class="primary production">▶ 생산 실행</button></div><p id="tutorial-message" class="tutorial-message"></p><div id="game-canvas" class="game-canvas" aria-label="공장 설계 공간"></div><p class="canvas-help">포트에서 드래그해 연결 · 빈 공간 또는 가운데 버튼으로 이동 · 휠로 확대 · Delete 삭제 · Ctrl+Z 되돌리기</p></section><aside class="right-panel panel"><div class="panel-heading"><span>선택 장치</span><small>연결 규칙과 효과</small></div><div id="module-detail"></div></aside></div><section class="result-panel panel"><div class="result-header"><div><span class="eyebrow">결과 비교 및 문제 해결</span><h2>생산 결과</h2></div><div class="result-actions"><button id="delivery" class="primary" disabled>📦 납품하기</button><button id="next-order" class="secondary" hidden>다음 의뢰 →</button></div></div><div id="result-content"></div></section></section><div id="toast" class="toast" role="status"></div></main>`;
}
