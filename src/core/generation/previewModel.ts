...
export function createPreviewKey(tags: GenerationTag[], band: PreviewQualityBand): string {
  return [
    "cat",
    tags.includes("style-fairytale") ? "fairytale" : "plain",
    tags.includes("no-hat") ? "no-hat" : "hat",
    tags.includes("centered-composition") ? "center" : "offset",
    tags.includes("sharpness") ? "sharp" : "soft",
...
export function buildPreviewModel(result: GenerationResult): PreviewModel {
  ...
  return {
    ...
    label: tags.has("quality-inspection") ? "검사 완료" : "제작 완료",
    effects,
    qualityBand,
  };
}
