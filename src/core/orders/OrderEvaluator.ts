...
export class OrderEvaluator {
  evaluate(order: OrderDefinition, result: GenerationResult): OrderEvaluation {
    ...
    if (!result.hasDelivery) issues.push(issue("missing-delivery", "배송대까지 연결된 완전한 생산 경로가 없습니다.", "delivery-bay", "배송대를 생산 라인의 끝에 연결하세요."));
    ...
    const displayedIssues = result.hasInspection ? issues : issues.slice(0, 2).map((item) => ({ ...item, detail: "품질 검사기를 연결하면 자세한 분석을 볼 수 있습니다." }));
    result.issues = displayedIssues;
    return { passed: issues.length === 0, issues: displayedIssues, summary: issues.length === 0 ? "주문 조건을 만족했습니다. 배송대로 납품할 수 있습니다." : displayedIssues[0]?.message ?? "생산 라인을 확인해 주세요." };
  }
}
...
function tagIssue(tag: string): GenerationIssue {
  const messages: Record<string, GenerationIssue> = {
    generator: issue("missing-generator", "그림 제작기가 주문서를 처리하지 못했습니다.", "image-maker", "주문서 입력기와 그림 제작기를 연결하세요."),
    "style-fairytale": issue("missing-style", "요청한 동화책 스타일이 적용되지 않았습니다.", "style-processor", "스타일 가공기를 그림 제작기 뒤에 연결하세요."),
    "no-hat": issue("missing-ban-list", "금지된 모자가 결과에 포함되었습니다.", "ban-list", "금지 목록 입력기를 생산 경로에 추가하세요."),
    "centered-composition": issue("missing-composition", "주인공이 화면 중앙에서 벗어났습니다.", "composition-planner", "구도 설계기를 연결해 보세요."),
    sharpness: issue("missing-sharpness", "이미지가 너무 흐립니다.", "sharpener", "선명화 장치를 그림 제작기 뒤에 연결하세요."),
    "quality-inspection": issue("missing-inspection", "품질 검사를 거친 완성품이 아닙니다.", "quality-checker", "품질 검사기를 배송대 앞에 연결하세요."),
  };
