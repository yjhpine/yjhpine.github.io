    ...
    story += [
        ...
        table(
            [
                ...
                ["핵심 경험", "손님 → 주문서 집기 → 왼쪽 선반 칩 → 입력기·슬롯 → 생산 → 출구 → 전달 → 정산 → 준비 타임(업그레이드) → 다음 라운드."],
                ["기본 스테이션", "입력기·슬롯(3)·생산기·출구는 하단 라인에 고정. 모듈 칩 선반은 왼쪽 세로. 플레이어가 꽂는 것은 중간 모듈 칩(+그림 제작기)."],
                ["비교의 재미", "X로 주문서/완성 이미지를 들여다보며 조건 충족·절차적 프리뷰·점수를 확인한다. 분석기(구매 시)는 칩 이름 없이 조건 일치만 보여 준다."],
        ...
        P(
            "라운드 플레이 → 손님 응대 → 칩 조합 생산(VRAM 소모) → 전달(납품 크레딧) → "
            "라운드 정산(등급·정산 크레딧) → 준비 타임(업그레이드 구매) → 다음 라운드 시작. "
            "새 게임(r00)은 준비 타임을 건너뛰고 바로 시작한다. 이어서 하기는 준비 타임부터."
        ),
        table(
            [
                ["플레이어 상황", "게임이 주는 답"],
        ...
        *header_bar("3. Overcooked vs 팩토리 — 구현된 선택"),
        ...
        table(
            [
                ...
                ["모듈 칩 6종 해금, 슬롯 3, 왼쪽 세로 선반 타입당 1개/라운드", "모듈 칩을 크레딧으로 사는 상점(칩 자체는 라운드 해금)"],
                ["납품 크레딧(+100/+50/+20/0) + 라운드 정산 크레딧", "전력/유지비/연구 트리"],
                ["준비 타임 공장 업그레이드 7종(처음부터 전부 구매 가능)", "인게임 터미널 상점, 라운드 해금 게이트"],
                ["VRAM 소프트 제약 + 효율 점수 S~D", "실모델 연동, 모바일 세로, 멀티플레이"],
                ["절차적 CSS 프리뷰, 로컬 세이브 v2(upgradeLevels 포함)", "네이티브/모바일 세로 전용 UI"],
            ],
            [86 * mm, 86 * mm],
        ),
        P(
    ...
    story += [
        ...
        table(
            [
                ...
                ["Z", "가장 가까운 대상과 상호작용(범위 70). 손님·입력기·슬롯·생산·출구·왼쪽 선반·퀵 선반·분석기·바닥. 찬 슬롯+칩 소지 시 스왑. 분석 모달이 열려 있으면 Z로 닫기."],
        ...
        *header_bar("3. 주방 스테이션과 생산 흐름"),
        P("맵 960×580. 상단 카운터(손님), 왼쪽 세로 칩 선반, 하단 컨베이어 생산 라인. 구매 시 우측 퀵 선반·분석기·주문 분석기."),
        table(
            [
                ["스테이션", "역할", "규칙"],
                ...
                ["칩 선반(왼쪽)", "해금 칩 재고", "세로 정렬. 라운드당 타입 1개. 집으면 ‘없음’, 라인 리셋 시 복구."],
                ["퀵 선반(우측)", "동선 단축", "칩 선반 확장 업그레이드 구매 후. 동일 재고 규칙."],
                ["분석기 / 주문 분석기", "조건 분석", "구매 후 배치. 칩 이름 비공개. Z로 열고 닫기."],
        ...
        table(
            [
                ["요소", "값 / 동작"],
                ["동시 대기", "최대 2명"],
                ["인내심", "기본 45초 × (1+커피머신 보너스). 대기 중 감소. r00(isTutorial)은 감소·이탈 없음."],
                ...
                ["납품 보상", "실패 +0. 성공 +100. 완벽(조건 전부 충족=통과 시) +50. 인내심≥50% +20. 최대 170. 라운드 정산 크레딧과 별도."],
            ],
            [36 * mm, 136 * mm],
        ),
        PageBreak(),
        *header_bar("5. VRAM · 생산 · 점수"),
        table(
            [
                ...
                ["생산 시간", "시뮬 처리시간×0.35(최소 0.8초)×슬로우다운×(1−생산기 업그레이드 감소)."],
                ["이동/딜레이", "작업화: 이동속도 배율. 작업 장갑: 상호작용 딜레이(기본 120ms) 감소."],
        ...
        P("<b>6. 준비 타임 · 공장 업그레이드</b>", "h2"),
        P(
            "업그레이드 구매는 라운드 플레이 중이 아니라 준비 타임(#prep-modal)에서만 한다. "
            "정산 후 ‘준비 타임 →’ 또는 메뉴 ‘이어서 하기’로 진입. 처음부터 7종 모두 목록에 보이며, "
            "잔액·최대 레벨만 검사한다(라운드 클리어 해금 게이트 없음). 세이브 upgradeLevels에 유지, 새 게임 시 초기화."
        ),
        table(
            [
                ["업그레이드", "최대", "가격(요지)", "효과"],
                ["작업화", "Lv.3", "100/250/500", "이동속도 +5/10/15%"],
                ["작업 장갑", "Lv.3", "100/250/500", "상호작용 딜레이 −10/20/30%"],
                ["커피머신", "Lv.3", "150/350/600", "손님 인내심 +10/20/30%"],
                ["생산기 업그레이드", "Lv.3", "200/400/700", "생산시간 −10/20/30%"],
                ["칩 선반 확장", "1회", "400", "우측 퀵 선반 최대 3칸"],
                ["분석기", "1회", "600", "결과물 조건 일치표(칩 이름 비공개)"],
                ["주문 분석기", "1회", "800", "주문서 구조화 조건(칩 이름 비공개)"],
            ],
            [40 * mm, 18 * mm, 36 * mm, 78 * mm],
        ),
        P("<b>7. 결과 생성</b>", "h2"),
        P(
        ...
        P(
            ...
            "불필요 칩은 VRAM만 늘려 효율·예산을 깎는다. 손에 든 아이템은 플레이어 스프라이트보다 항상 위에 그린다.",
            "note",
        ),
    ]
    ...
    story += [
        ...
        *header_bar("3. 주문(O01~O06)과 모듈 칩"),
        ...
        P(
            ...
            "모듈 학습은 해금 모달(unlockTutorial)에 맡긴다. "
            "모듈 칩 자체는 라운드 해금으로만 풀리고, 크레딧 상점은 공장 업그레이드(준비 타임)에만 쓴다.",
            "note",
        ),
        P("<b>4. 선반·슬롯 규칙</b>", "h2"),
        P(
            "왼쪽 세로 선반에 라운드당 해금 칩 타입마다 재고 1개. 집으면 소진, 「라인 비우기」로 복구. "
            "슬롯 3칸, 같은 칩 스왑 거부. 칩 선반 확장 시 우측 퀵 선반도 동일 재고를 공유한다."
        ),
        P("<b>5. 에셋 기준 (제품 화면)</b>", "h2"),
        P(
            "Cozy/Cute 픽셀 공장: 타일·컨베이어·스테이션·칩·손님(토끼/개/햄스터/오리). "
            "플레이어는 Cat_* 32×32 시트(idle/walk + 운반). 들여다보기 프리뷰는 사진 팩/절차적 폴백. "
...
def doc_04():
    ...
    story += [
        table(
            [
                ["영역", "표시"],
                ["메뉴", "새 게임(r00 바로 시작) / 이어서 하기(준비 타임) / 진행 요약. WASD·Z·C·X·준비 타임 업그레이드 안내."],
                ...
                ["중앙", "Phaser 960×580 주방. 상단 손님·왼쪽 칩 선반·하단 생산 라인. 하이라이트·[Z]·가이드·프로그레스/COMPLETE."],
                ["토스트", "상호작용 결과·납품 보상 breakdown·튜토리얼 힌트·이탈 등 짧은 알림."],
            ],
            [32 * mm, 140 * mm],
        ),
        P("<b>2. 들여다보기 (X)</b>", "h2"),
        table(
            [
                ...
                ["완성 이미지", "요청 문구 + 프리뷰 + 조건 충족/미달 요약 + 점수·이번 생산 VRAM."],
            ],
            [40 * mm, 132 * mm],
        ),
        P("<b>3. 해금·정산·준비 타임 UI</b>", "h2"),
        table(
            [
                ["상황", "UI"],
                ["일반 라운드, 미소개 칩", "중앙 블로킹 모달(#unlock-tutorial). Phaser pause. ‘알겠어요’ → introducedModuleIds 저장 후 재개."],
                ["r00 튜토리얼", "해금 모달 생략. 사이드 단계 문구 + 월드 가이드만."],
                ["라운드 정산", "등급·점수 내역·정산 크레딧. CTA는 ‘준비 타임 →’(다음 라운드 활성화 후 상점)."],
                ["준비 타임", "#prep-modal. 다음 라운드 요약·보유 크레딧·업그레이드 목록·구매·‘라운드 시작 →’."],
                ["분석기", "#analysis-modal. 조건 일치/요구 목록(칩 이름 없음). 닫기 버튼 또는 Z."],
            ],
            [48 * mm, 124 * mm],
        ),
        PageBreak(),
        *header_bar("4. 인게임 피드백 원칙"),
        table(
            [
                ...
                ["휴대 아이템", "주문서/칩/사진은 플레이어 스프라이트보다 항상 위에 표시."],
            ],
            [40 * mm, 132 * mm],
        ),
        P(
    ...
    story += [
        table(
            [
                ...
                ["자원", "납품·정산 크레딧 + 준비 타임 업그레이드 소모, VRAM 예산", "칩 구매 상점·유지비·연구 트리"],
                ["튜토리얼", "r00 단계 잠금 + 해금 모달 + 준비 타임", "라이프/E키/별도 퀘스트 엔진"],
                ["결과", "결정적 GenerationSimulator + 프리뷰 사진 팩", "실제 생성 API"],
            ],
            [28 * mm, 84 * mm, 60 * mm],
        ),
        ...
        table(
            [
                ["컴포넌트", "책임"],
                ["KitchenSession", "운반·손님·슬롯·생산·VRAM·이탈·라운드 종료·납품 보상. 업그레이드 배율(인내심·생산시간)."],
                ["KitchenScene", "Phaser 960×580 이동·대시·Z/X/C·좌측 선반·하단 라인·퀵/분석기·가이드·연출."],
                ["TutorialGuide", "r00 단계 FSM, 허용 대상, 힌트. 세션 규칙을 재설계하지 않는 얇은 레이어."],
                ["UIController", "메뉴·HUD·해금·정산·준비 타임 상점·분석 모달(Z 닫기)·세이브."],
                ["RoundScoreService", "ideal VRAM·100점 환산·등급·정산 크레딧."],
                ["ProgressionService / SaveService", "activeRoundId, 해금·소개 칩, bestScore, upgradeLevels, localStorage v2."],
                ["upgrades.ts / analysisView", "업그레이드 정의·효과·납품 보상표·분석 행(칩 이름 비공개)."],
        ...
        *header_bar("3. 데이터"),
        table(
            [
                ...
                ["ModuleDefinition", "vramCost, unlockTutorial, iconKey, (price는 칩 상점에 미사용)"],
                ["UpgradeDefinition", "levels[], maxLevel, (unlockAfterRoundId는 데이터만·런타임 해금 게이트 없음)"],
                ["OrderDefinition", "prompt, requiredTags, minimumScores, reward, …"],
                ["SaveData v2", "credits, completedRoundIds, unlockedModuleIds, introducedModuleIds, tutorialStage, activeRoundId, bestRoundScores, upgradeLevels"],
            ],
            [40 * mm, 132 * mm],
        ),
        P("<b>4. 검증</b>", "h2"),
        table(
            [
                ...
                ["npm run test", "Vitest(코어·튜토리얼·세이브·점수·업그레이드·납품 보상 등)"],
                ["npm run build", "dist/ 생성, Pages 배포"],
                ["수동", "새 게임→r00→정산→준비 타임 구매→r01, Z/C/X, 분석기 Z 닫기, 이탈 정리, 해금 모달"],
            ],
            [40 * mm, 132 * mm],
        ),
        ...
        P(
            ...
            "X 프리뷰로 성공/실패를 말하고, 준비 타임에서 번 크레딧으로 공장이 좋아지는 피드백을 느끼면 MVP 합격으로 본다.",
            "note",
        ),
        P(
            "<b>폐기된 구기획 요약</b><br/>"
            ...
            "인게임 업그레이드 터미널(E/Space), 납품 reward×0.35/×0.1, 맵 960×720·하단 선반, "
            "업그레이드 라운드 해금 게이트 — 모두 현재 빌드와 맞지 않아 이 개정에서 제거·갱신했다.",
            "body",
        ),
    ]
